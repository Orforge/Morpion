...
# Class Computer - Implémentation de Player pour l'ordinateur
class Computer(Player):

    def play(self, game):
        return self.bestMove(game, game.table, game.size, self.sign)[0]

    # Sélectionner la meilleure possibilitée de jeu
    def bestMove(self, game, table, size, sign):
        # On recupere le signe de l'adverse pour nos calculs
        other = ("X" if sign == "O" else "O")
        # On cree une liste vide pour y ajouter nos possibilitees avec leur score
        moves = list()

        # On parcours le tableau pour classer chaque possibilitée
        for x in range(size):
            for y in range(size):
                ...
                if table[x][y] == "*":
                    ...
                    # Si il permet de gagner, on le grade 1
                        ...
                        score = 1
                    # Sinon, on le grade avec l'opposé du score pour le joueur adverse
